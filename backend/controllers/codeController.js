...
exports.runCode = asyncHandler(async (req, res) => {
  ...
  await runQueue.add(
    'run',
    {
      ...
      uuid: submission.jobUUID
    },
    { attempts: 1}
  );
  res.json({ success: true, jobId: submission._id });
});
...
exports.submitCode = asyncHandler(async (req, res) => {
  ...
  if (!questionId || !language || !code)
    return res.status(400).json({ error: 'questionId, language and code required' });
  ...
  res.json({ success: true, submissionJobId: submission._id });
});